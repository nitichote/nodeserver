...
router.get('/hstation', async (req: Request, res: Response) => {
  const db = req.db9;
  let sql = '';
    sql = `SELECT r.*,h.off_id,h.off_name,h.off_type,h.provid,h.pointx,h.pointy,h.subdistid FROM r9_health_station r inner join  hospitalcpho  h on r.hospcode = h.off_id and h.provid=36 and LENGTH(r.latitude) >0 and LENGTH(r.longtitude) >0
    `;
  try {
    const raw = await db.raw(sql);
    res.send({ ok: true, message: raw[0] });
...
router.get('/icdsubgroup', async (req: Request, res: Response) => {
  const db = req.db9;
  let sql = '';
    sql = `select * from icdsubgroup`;
  try {
    const raw = await db.raw(sql);
    res.send({ ok: true, message: raw[0] });
  } catch (error) {
...
router.get('/icdspec', async (req: Request, res: Response) => {
  const db = req.db9;
  let sql = '';
    sql = `select * from icdsubgroup`;
  try {
...
router.get('/caticd', async (req: Request, res: Response) => {
  const db = req.db9;
  let sql = '';
    sql = `select * from caticd`;
  try {
    const raw = await db.raw(sql);
    res.send({ ok: true, message: raw[0] });
  } catch (error) {
...
router.get('/icdlist', async (req: Request, res: Response) => {
  const db = req.db9;
  let sql = '';
    sql = `select * from icd10tm_dental c inner join icdgroup g on c.group = g.gcode order by c.gsub`;
  try {
    const raw = await db.raw(sql);
    res.send({ ok: true, message: raw[0] });
  } catch (error) {
