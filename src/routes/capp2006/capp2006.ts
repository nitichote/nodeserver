...
router.get('/txcodes', async (req: Request, res: Response) => {
  const db = req.db7;
  try {
    const raw = await db.raw(`select t.*,g.*,left(s.scode,4) as scode,s.swork from ctreat2006 t inner join ctxgroup g on left(t.workcode,2) = g.gcode inner join ctxsubgroup s on left(t.workcode,4) = left(s.scode,4)
    `);
    res.send({ ok: true, message: raw[0] });
  } catch (error) {
    res.send({
